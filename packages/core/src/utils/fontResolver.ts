...
const FONT_MAPPINGS: Record<string, FontMapping> = {
  ...
  // entries. `applyThemeFontLang` resolves empty `<a:ea>` slots to these native
  // names (not the romanized "SimSun"/"Malgun Gothic"), so map them to the same
  // Noto webfonts — otherwise Chinese/Korean runs wrap but render and measure
  // with whatever font the viewer's OS happens to have. (CJK has no letter
  // case, so `.toLowerCase()` leaves these keys unchanged.)
 ...
 * FONT_MAPPINGS. Word writes either the native typeface name (the theme path,
 * e.g. `新細明體`) or the romanized name (run `rFonts`, e.g. `PMingLiU`); both
 * must resolve to the same Noto family + CSS stack + category. Aliasing keeps a
 * single set of CJK entries instead of duplicating each under both spellings.
 * (Keys already present directly in FONT_MAPPINGS — `simsun`, `simhei`,
 * `malgun gothic`, `ms mincho`, `ms gothic` — are intentionally omitted.)
 */
